...
class GetBackupsToRemoveTest(unittest.TestCase):
    ...
    def test_parse_date_yyyy_mm_dd(self):
        ...
        date_format = "%Y-%m-%d"
        backup_date = backup_autoremove.parse_date(regex_pattern, file, date_format)
    ...
    def test_parse_date_empty_pattern(self):
        ...
        with self.assertRaises(ValueError):
            backup_autoremove.parse_date(regex_pattern, file, date_format)

    def test_parse_date_no_match(self):
        regex_pattern = "[0-9]{4}-[0-9]{2}-[0-9]{2}$"
        file = "someOtherfile.txt"
        date_format = "%Y-%m-%d"
        self.assertEqual(False, backup_autoremove.parse_date(regex_pattern, file, date_format))
